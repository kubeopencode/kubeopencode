...
// +kubebuilder:validation:XValidation:rule="(has(self.value) && size(self.value) > 0) || (has(self.valueFrom) && has(self.valueFrom.secretKeyRef))",message="either a non-empty value or valueFrom.secretKeyRef must be specified"
