...
#### OCI Skill Artifact Layout

Each skill is packaged as a standard **OCI image manifest** (not an ORAS artifact manifest) to maximize registry compatibility. The artifact contains a single layer — a tar.gz of the skill directory:

```
OCI Image Manifest (application/vnd.oci.image.manifest.v1+json)
├── config: application/vnd.kubeopencode.skill.config.v1+json
│   └── {"name": "kubernetes-ops", "version": "1.0.0", ...}  # metadata.yaml as JSON
└── layers:
    └── application/vnd.kubeopencode.skill.content.layer.v1.tar+gzip
        └── kubernetes-ops/
            ├── SKILL.md
            └── metadata.yaml
```

Using the standard OCI image manifest (rather than ORAS artifact manifest) ensures compatibility with all OCI registries (Zot, Harbor, GHCR, ECR, ACR). The custom media types on config and layer allow OCI-aware tools to distinguish skill artifacts from container images.

**One skill per OCI artifact** is the default packaging model. Each OCI reference (e.g., `zot.svc:5000/skills/kubernetes-ops:1.0.0`) contains exactly one skill. This keeps the model simple: one artifact = one skill = one version tag. The `OCISkillSource.Images` list allows referencing multiple skills within a single `SkillSource` entry for grouping convenience.

The `Names` field on `OCISkillSource` exists for a **bundle** scenario: a single OCI artifact containing multiple skill directories (e.g., a team publishes all their skills as one artifact). In the common one-skill-per-artifact case, `Names` can be omitted. When omitted, all skill directories found in the artifact are mounted.

#### `skill-init` Container Implementation

`skill-init` is a new subcommand of the **unified `kubeopencode` binary** (`cmd/kubeopencode/skill_init.go`), consistent with the existing `git-init`, `context-init`, and `url-fetch` subcommands. This means `crane` (from `go-containerregistry`) is compiled into the unified binary as a Go library dependency — no separate binary or image needed.

The `skill-init` container:

1. Receives OCI references via environment variable `SKILL_OCI_IMAGES` (JSON array of image references)
2. For each reference, calls `crane.Export()` (or `remote.Image()` + layer extraction) to pull and unpack the artifact
3. Extracts the skill directory to `/skills/{source-name}/{skill-name}/`
4. If `Names` filter is specified, only extracts matching subdirectories
5. Uses `SecretRef` credentials (mounted as Docker config JSON) for authenticated pulls

```yaml
# Example skill-init init container (generated by pod_builder.go)
- name: skill-init-registry-skills
  image: ghcr.io/kubeopencode/kubeopencode:latest  # Unified binary
  command: ["/kubeopencode", "skill-init"]
  env:
    - name: SKILL_OCI_IMAGES
      value: '["zot.svc:5000/skills/kubernetes-ops:1.0.0","zot.svc:5000/skills/security-review:2.0.0"]'
    - name: SKILL_DEST_DIR
      value: /skills/registry-skills
  volumeMounts:
    - name: skills-registry-skills
      mountPath: /skills/registry-skills
```

#### Controller Pipeline Changes (`skill_processor.go`)

The existing `processSkills()` function converts `SkillSource` items into `gitMount` objects. With OCI support, the pipeline generalizes to handle both source types:

1. **`processSkills()`** iterates over `SkillSource` items and branches:
   - If `source.Git != nil` → produces a `gitMount` (existing behavior, unchanged)
   - If `source.OCI != nil` → produces an `ociMount` (new struct: init container spec + volume mount)
2. **`buildPod()` / `buildDeployment()`** creates init containers for both types:
   - `git-init-{name}` containers for Git sources (existing)
   - `skill-init-{name}` containers for OCI sources (new)
3. **Skill path injection** remains the same — both source types produce paths under `/skills/{source-name}/` which are injected into `opencode.json` via `injectSkillsIntoConfig()`

This is an additive change to the pipeline — all existing Git skill behavior is untouched.

**API changes in KubeOpenCode (minimal):**

```go
// SkillSource gains an OCI option alongside Git.
// The existing XValidation rule `has(self.git)` is replaced with a mutual-exclusion rule:
// +kubebuilder:validation:XValidation:rule="has(self.git) || has(self.oci)",message="either git or oci source must be specified"
// +kubebuilder:validation:XValidation:rule="!(has(self.git) && has(self.oci))",message="git and oci sources are mutually exclusive"
...
- **Access control model** — Unlike Zot (which uses OCI path conventions for namespace isolation), Verdaccio serves as a **cluster-wide shared cache** without namespace-level isolation. All namespaces see the same set of plugins. This is acceptable because plugins are curated by administrators — the Registry server controls which packages are published to Verdaccio. Per-namespace plugin visibility, if needed, is handled at the **Agent/AgentTemplate level** (teams simply don't reference plugins they shouldn't use), not at the registry level. Namespace-scoped npm registries (one Verdaccio per namespace) would create N identical caches and significant ops overhead for negligible security benefit. Fine-grained plugin access control is deferred to Phase 3 (RBAC).
...
5. **Built-in content versioning and maintenance**
   - Built-in skills and images follow **independent semver** (not tied to KubeOpenCode release versions). Each skill has its own `version` in `metadata.yaml`.
   - CI publishes OCI artifacts with both version tags (`skills/golang:1.0.0`) and a `:latest` tag tracking the most recent release.
   - Quality: built-in skills are reviewed through the standard PR process. Community contributions welcome under `registry/skills/`.
   - **Phase 1 air-gap note**: In Phase 1 there is no in-cluster Zot. Built-in skills are hosted on GHCR, so pulling them requires internet access. Air-gapped users can still use the existing `git` skill source (clone from an internal Git mirror) or manually push OCI skill artifacts to any internal OCI registry using `oras push` or `crane push`. Full air-gap support (in-cluster Zot + Verdaccio) arrives in Phase 2 with the Registry deployment.
...
2. **Supply chain security** — Cosign/Sigstore signing for built images and skill OCI artifacts. Optional signature verification on pull (via Kyverno/Gatekeeper policy integration or controller-level verification). Mutable tags (e.g., `:latest`) can be prohibited by policy; digest-only references enforced in production namespaces
3. **Audit trail** — Track asset changes with timestamps and user attribution
4. **Scheduled rebuilds** — Cron-based rebuild for base image security patches
5. **Multi-cluster sync** — Zot replication for multi-cluster deployments
6. **Update notifications** — Detect when AgentTemplate references outdated skill/image digests
