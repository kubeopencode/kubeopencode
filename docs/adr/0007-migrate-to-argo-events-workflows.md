...
KubeTask initially implemented several CRDs for workflow orchestration and event-driven triggers:
...
Additionally, KubeTask included:
...
4. **Scope Creep**: KubeTask's value proposition is the Task/Agent abstraction for AI workloads. Workflow orchestration and event handling are orthogonal concerns.

5. **Unused Features**: Session persistence and humanInTheLoop were experimental features that saw limited adoption.

## Decision

**Migrate workflow orchestration and webhook handling to Argo Workflows and Argo Events, while keeping KubeTask focused on the core Task and Agent CRDs.**
...
3. **Webhook Server**:
   ...
   - cmd/kubetask/webhook.go

4. **Session Features**:
   - humanInTheLoop in Agent spec
   - SessionPVC in KubeTaskConfig
...
1. **Core CRDs**:
   ...
   - KubeTaskConfig (simplified to systemImage only)
...
Argo Events creates KubeTask Tasks (not Argo Workflows directly), preserving the Task/Agent abstraction:
...
This pattern:
- Keeps KubeTask as the execution layer for AI tasks
...
3. **Clear Focus**: KubeTask has a clearer value proposition - AI task execution
...
3. The Sensor should create KubeTask Tasks (same agentRef, description, contexts)
